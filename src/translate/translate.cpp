...
void Translater::action_ARRAY_ASSIGN(Parser& parser)
{
    ...
    code_manager.generate_code(ARRAY_ASSIGN, vol1.addr, vol.addr, parser.smanager);
}
...
void Translater::action_ASSIGN(Parser& parser)
{
    ...
    code_manager.generate_code(ASSIGN, id.addr, vol.addr, parser.smanager);
}
...
void Translater::action_BOOL2(Parser& parser)
{
    ...
    auto r = gen_3_addr(parser, op.get_token(), value1.addr, value2.addr);
...
void Translater::action_ELSEIF(Parser& parser)
{
    ...
    addr_type jump_false = parser.smanager.install_value(0);
    code_manager.generate_code(GOTO, jump_false, parser.smanager);
...
void Translater::action_WHILE(Parser& parser)
{
    ...
    // gencode
    code_manager.generate_code(GOTO, jump_back, parser.smanager);
...
void Translater::action_F3_FLAG(Parser& parser)
{
    ...
    pop_all(parser);
    size_t jmp = code_manager.line_nums_now();
    auto addr_goto = backfill_false.top();
    backfill_false.pop();
    parser.smanager.value_assignment(addr_goto, (int)jmp);
    code_manager.merge_code();
    // backfill ture_jump
    size_t back_bool = code_manager.line_nums_now();
    for (int i = 0; i < bool_nums.top(); ++i) {
        auto addr = backfill_true.top();
        backfill_true.pop();
        parser.smanager.value_assignment(addr, (int)back_bool);
        back_bool -= 2;
    }
}